            ...
            <div class="panel-wrapper collapse in">
                <div class="panel-body">
                    <form id="CategoryForm" enctype="multipart/form-data">
                        <div class="form-wrap">
                            <input type="hidden" name="addon_sub_category_id" id="addon_sub_category_id" value="{{ $addon_sub_category->id }}">
                            {{ csrf_field() }}
                            <h6 class="txt-dark capitalize-font"><i class="icon-list mr-10"></i>About Addon Sub Category</h6>
                            <hr>
                            <div class="row">
                                <!--/span-->
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="control-label mb-10">Addon Sub Category Name</label>
                                        <input class="form-control" name="name" value="{{ $addon_sub_category->name }}" required type="text" placeholder="Enter Sub Category name">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="control-label mb-10">Addon Categories</label>
                                        <select name="category_id" class="form-control selectpicker btn-outline-none" data-style="btn-default btn-outline">
                                            @forelse ($categories as $category)
                                            <option value="{{ $category->id }}" {{ $addon_sub_category->category_id = $category->id ? "selected" :"" }}>{{ $category->name }}</option>
                                            @empty
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="restaurant_id" class="form-control selectpicker btn-outline-none" data-style="btn-default btn-outline">
                                            ...
                                            <option value="{{ $restaurant->id }}" {{ $addon_sub_category->restaurant_id = $restaurant->id ? "selected" :"" }}>{{ $restaurant->name }}</option>
                                            @empty
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="img-upload-wrap">
                                        <div id="imagePreview">
                                            <img src="{{ url('/image/addon_sub_category/'.$addon_sub_category->image) }}" width="80">
                                        </div>
...
<script>
    $("#CategoryForm").on("submit", function(e) {
        ...
        var id = $("#addon_sub_category_id").val();
        // console.log(form);
        $.ajax({
            url: "/admin/edit_addon_sub_category/" + id,
            method: "POST",
            data: formData,
            ...
            success: function(data) {
                ...
                if (data.message != "") {
                    ...
                    $("#spinner").hide();
                    setTimeout(function() {
                        window.location.assign('/admin/addon-sub-categories')
                    }, 1500)
                }
            },
            error: function(data) {
